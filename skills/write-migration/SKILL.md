...
```bash
npx cascade migrate            # apply pending migrations
npx cascade migrate:rollback   # undo the last batch
npx cascade migrate:list       # which migrations have been executed
...
Every pending migration's SQL is pooled and sorted **globally**, by three keys in order:

1. **Phase** — the dependency-safe DDL order, derived from the statement itself: extensions/types (1), `CREATE TABLE` (2), `ADD COLUMN` (3), foreign keys (4), drops/alters (5). A `CREATE TABLE` from a newer migration therefore still runs before an older migration's `ALTER`.
2. **`createdAt`** — parsed from the filename prefix.
3. **Migration name** — alphabetical, only when the timestamps can't decide.

Because phase wins, cross-migration ordering matters most for statements that share a phase — two migrations both altering the same table, for instance.

:::caution[`MM-DD` vs `DD-MM` is a guess]
The parser tells the two apart by whether the first field is greater than 12 — a month can't be. When both fields are ≤ 12 the stamp is genuinely ambiguous and it assumes `MM-DD`. Two migrations generated on `01-08` and `08-01` of the same year sort by that assumption rather than by fact. If exact ordering between two such files matters, make it unambiguous — a differing hour is enough.
:::
