...
import { TableIndexInformation } from "./driver-blueprint.contract.mjs";
 ...
 */
type ColumnType = "string" | "char" | "text" | "mediumText" | "longText" | "integer" | "smallInteger" | "tinyInteger" | "bigInteger" | "float" | "double" | "decimal" | "boolean" | "date" | "dateTime" | "timestamp" | "time" | "year" | "json" | "binary" | "uuid" | "ulid" | "ipAddress" | "macAddress" | "point" | "polygon" | "lineString" | "geometry" | "vector" | "enum" | "set" | "arrayInt" | "arrayBigInt" | "arrayFloat" | "arrayDecimal" | "arrayBoolean" | "arrayText" | "arrayDate" | "arrayTimestamp" | "arrayUuid" | "arrayJson";
/**
 * Column definition used when adding or modifying columns.
 */
type ColumnDefinition = {
  /** Column name */name: string; /** Column data type */
  type: ColumnType; /** Length for string/char types */
  length?: number; /** Precision for decimal types */
  precision?: number; /** Scale for decimal types */
  scale?: number; /** Whether the column allows NULL values */
  nullable?: boolean; /** Default value for the column (can be a primitive, SQL string, or {__type: 'CURRENT_TIMESTAMP'}) */
  defaultValue?: unknown; /** MySQL: ON UPDATE CURRENT_TIMESTAMP */
  onUpdateCurrent?: boolean; /** Whether this is a primary key */
  primary?: boolean; /** Whether this column auto-increments */
  autoIncrement?: boolean; /** Whether this column must be unsigned (numeric only) */
  unsigned?: boolean; /** Whether this column has a unique constraint */
  unique?: boolean; /** Column comment/description */
  comment?: string; /** Enum/set values */
  values?: string[]; /** Vector dimensions (for vector type) */
  dimensions?: number; /** Whether defaultValue should be treated as raw SQL (true) or escaped literal (false) */
  isRawDefault?: boolean; /** Position this column after another column */
  after?: string; /** Position this column as the first column in the table */
  first?: boolean; /** Generated column configuration */
  generated?: {
    /** SQL expression to compute the value */expression: string; /** true = STORED (persisted), false = VIRTUAL (computed on read) */
    stored: boolean;
...
type IndexDefinition = {
  /** Index name (auto-generated if not provided) */readonly name?: string; /** Columns included in the index */
  readonly columns: string[]; /** Whether this is a unique index */
  readonly unique?: boolean; /** Index type (driver-specific) */
  readonly type?: string; /** Partial index condition */
  readonly where?: Record<string, unknown>; /** Whether to create sparse index (MongoDB) */
  readonly sparse?: boolean; /** Sort direction for each column */
  readonly directions?: Array<"asc" | "desc">; /** Expression-based index (PostgreSQL) - e.g., ['lower(email)', 'upper(name)'] */
  readonly expressions?: string[]; /** Covering index columns (PostgreSQL INCLUDE clause) */
  readonly include?: string[]; /** Create index concurrently without blocking writes (PostgreSQL) */
...
type FullTextIndexOptions = {
  /** Index name */readonly name?: string; /** Language for text analysis */
  readonly language?: string; /** Field weights for relevance scoring */
...
type GeoIndexOptions = {
  /** Index name */readonly name?: string; /** Index type: "2dsphere" (default) or "2d" */
  readonly type?: "2dsphere" | "2d"; /** Minimum bound for 2d index */
  readonly min?: number; /** Maximum bound for 2d index */
  readonly max?: number;
};
/**
...
type VectorIndexOptions = {
  /** Vector dimensions (e.g., 1536 for OpenAI) */readonly dimensions: number; /** Similarity metric */
  readonly similarity?: "cosine" | "euclidean" | "dotProduct"; /** Index name */
  readonly name?: string; /** Number of lists/clusters (IVF) */
...
type ForeignKeyDefinition = {
  /** Constraint name */readonly name?: string; /** Local column name */
  readonly column: string; /** Referenced table */
  readonly referencesTable: string; /** Referenced column */
  readonly referencesColumn: string; /** Action on delete */
  readonly onDelete?: "cascade" | "restrict" | "setNull" | "noAction"; /** Action on update */
 ...
 */
type MigrationDriverFactory = (source: DataSource | DriverContract) => MigrationDriverContract;
//#endregion
export { ColumnDefinition, ColumnType, ForeignKeyDefinition, FullTextIndexOptions, GeoIndexOptions, IndexDefinition, MigrationDriverContract, MigrationDriverFactory, VectorIndexOptions };
//# sourceMappingURL=migration-driver.contract.d.mts.map