import { ModelDefaults } from "../types.mjs";
import { DriverBlueprintContract } from "./driver-blueprint.contract.mjs";
import { MigrationDriverContract } from "./migration-driver.contract.mjs";
...
type InsertResult<TDocument = unknown> = {
  document: TDocument;
};
...
type UpdateOperations = {
  /** Set field values */$set?: Record<string, unknown>; /** Remove/unset fields (MongoDB: delete field, SQL: SET NULL) */
  $unset?: Record<string, 1 | true>; /** Increment numeric fields */
  $inc?: Record<string, number>; /** Decrement numeric fields */
  $dec?: Record<string, number>; /** Push to arrays (NoSQL only, SQL drivers may ignore) */
  $push?: Record<string, unknown>; /** Pull from arrays (NoSQL only, SQL drivers may ignore) */
...
type CreateDatabaseOptions = {
  /** Database encoding (PostgreSQL: UTF8, LATIN1, etc.) */encoding?: string; /** Template database (PostgreSQL) */
  template?: string; /** Locale/collation settings */
  locale?: string; /** Owner of the new database */
  owner?: string;
};
/**
...
type DropDatabaseOptions = {
  /** Force drop even if there are active connections */force?: boolean; /** Skip error if database doesn't exist */
  ifExists?: boolean;
};
//#endregion
export { CreateDatabaseOptions, DriverContract, DriverEvent, DriverEventListener, DriverTransactionContract, DropDatabaseOptions, InsertResult, TransactionContext, UpdateOperations, UpdateResult };
//# sourceMappingURL=database-driver.contract.d.mts.map