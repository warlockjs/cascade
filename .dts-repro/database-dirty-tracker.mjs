...
function flatten(object, separator = ".", keepNestedOriginalObject = false, parent, root = {}) {
	...
	for (const key of Object.keys(object)) {
		...
		if (Array.isArray(value) && value.length === 0 || typeof value === "function") root[keyChain] = value;
		else if (canBeFlatten(value)) {
			if (keepNestedOriginalObject) root[keyChain] = value;
			flatten(value, separator, keepNestedOriginalObject, keyChain, root);
		} else root[keyChain] = value;
	}
	return root;
}
/**
...
* ```
*/
var DatabaseDirtyTracker = class {
	/**
	...
	*/
	getDirtyColumnsWithValues() {
		...
		for (const column of this.dirtyColumns) {
			const hasCurrent = this.currentFlattened[column] !== void 0 || column in this.currentFlattened;
			result[column] = {
				oldValue: this.initialFlattened[column],
				newValue: hasCurrent ? this.currentFlattened[column] : void 0
			};
		}
		return result;
	}
	...
	*/
	unset(columns) {
		...
		for (const path of targets) this.deleteFromRaw(path);
		this.currentFlattened = this.flattenData(this.currentRaw);
		this.updateDirtyState();
	}
	/**
	...
	*/
	updateDirtyState() {
		this.dirtyColumns.clear();
		this.removedColumns.clear();
		const keys = new Set([...Object.keys(this.initialFlattened), ...Object.keys(this.currentFlattened)]);
		for (const key of keys) {
			const hasCurrent = this.currentFlattened[key] !== void 0 || key in this.currentFlattened;
			const hasInitial = this.initialFlattened[key] !== void 0 || key in this.initialFlattened;
			if (!hasCurrent && hasInitial) this.removedColumns.add(key);
			const initialValue = this.initialFlattened[key];
			if (!areEqual(initialValue, hasCurrent ? this.currentFlattened[key] : void 0)) this.dirtyColumns.add(key);
		}
	}
	/**
	...
	*/
	deleteFromRaw(path) {
		...
		for (let index = 0; index < segments.length - 1; index += 1) {
			if (container === void 0 || container === null) return;
		...
		if (Array.isArray(container)) {
			...
			if (!Number.isNaN(numericIndex)) container.splice(numericIndex, 1);
			return;
		}
		if (typeof container === "object") delete container[lastSegment];
	}
	/**
	...
	*/
	resolveSegment(container, segment) {
		...
		if (container && typeof container === "object") return container[segment];
	}
	/**
...
//#endregion
export { DatabaseDirtyTracker };

//# sourceMappingURL=database-dirty-tracker.mjs.map