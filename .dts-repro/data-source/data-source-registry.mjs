...
var DataSourceRegistry = class {
	...
	*/
	register(options) {
		...
		if (isNewDefault) this.defaultSource = source;
		this.events.emit("registered", source);
		if (isNewDefault) this.events.emit("default-registered", source);
	...
	*/
	...
	get(name) {
		...
		if (contextSource) {
			if (contextSource instanceof DataSource) return contextSource;
			const override = this.sources.get(contextSource);
			if (!override) throw new MissingDataSourceError(`Data source "${contextSource}" is not registered (context override).`, contextSource);
			return override;
		}
		if (name != null) {
			const source = this.sources.get(name);
			if (!source) throw new MissingDataSourceError(`Data source "${name}" is not registered.`, name);
			return source;
		}
		if (!this.defaultSource) throw new MissingDataSourceError("No default data source registered.");
...
//# sourceMappingURL=data-source-registry.mjs.map